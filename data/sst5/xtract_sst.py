...
with open("sst_train_phrases.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in train:
		wr.writerow([train[key],key])
with open("sst_train_sentences.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in train_sent:
		wr.writerow([train_sent[key],key])
with open("sst_test.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in test:
		wr.writerow([test[key],key])
with open("sst_dev.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in dev:
		wr.writerow([dev[key],key])

with open("sst5-train-phrases.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in train:
		x=train[key]
		if x<=0.2:
			...
			x='very pos'
		wr.writerow([key,x])
with open("sst5-train.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in train_sent:
		x=train_sent[key]
		if x<=0.2:
			...
			x='very pos'
		wr.writerow([key,x])
with open("sst5-test.csv","w") as f:
	wr=csv.writer(f,delimiter=',')
	for key in test:
		x=test[key]
		if x<=0.2:
